...
func test_door_opens_and_emits_signal_with_keycard():
	...
	# Player 1 + a keycard owned by player 1
	var player := FakePlayer.new()
	add_child_autofree(player)

	var keycard := FakeKeycard.new()
	add_child_autofree(keycard)

	# Simulate server handling the request to open
	door.rpc_request_open(1)
	await get_tree().process_frame

	assert_true(door.is_open, "Door should be open when requester holds the keycard")
	assert_true(was_emitted, "lab_door_opened should be emitted when door opens")
...
func test_door_stays_closed_without_keycard():
	...
	# Player 2, but NO keycard owned by player 2 in the scene
	var player2 := FakePlayer2.new()
	add_child_autofree(player2)

	# Ask server to open for player 2 (should fail validation)
	door.rpc_request_open(2)
	await get_tree().process_frame

	assert_false(door.is_open, "Door should remain closed when requester lacks keycard")
	assert_false(was_emitted, "Signal should not emit if door didn't open")
